...
const Navbar = ({ isLoggedIn }) => {
  const [isPostScreamDialogOpened, setIsPostScreamDialogOpened] = useState(false);
  return (
    <AppBar>
      <ScreamPostDialog
        open={isPostScreamDialogOpened}
        handleClose={() => { setIsPostScreamDialogOpened(false); }}
      />
            ...
            ? (
              <>
                ...
                <IconButtonWithTooltip
                  tooltipText="Post a scream"
                  icon={<AddIcon className="text-white" />}
                  handleClick={() => { setIsPostScreamDialogOpened(true); }}
                />
